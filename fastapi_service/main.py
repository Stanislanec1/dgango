...
async def upload_document(file: UploadFile = File(...), db: Sessionlocal = Depends(get_db)):
    ...
    existing = db.query(models.Document).filter(models.Document.filename == file.filename).first()
    if existing:
        raise HTTPException(status_code=400, detail=f"Файл {file.filename} уже существует")
